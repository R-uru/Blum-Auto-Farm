...
# Check interval for the "Play" button in seconds
CHECK_INTERVAL = 5
...
class AutoClicker:
    ...
    def check_and_click_play_button(self, sct, monitor):
        ...
        if current_time - self.last_check_time >= CHECK_INTERVAL:
            ...
            templates = [
                cv2.imread(os.path.join("template_png", "play_ruru.png"), cv2.IMREAD_GRAYSCALE),
                ...
                cv2.imread(os.path.join("template_png", "captcha.png"), cv2.IMREAD_GRAYSCALE)  # Add a new template
            ]
            ...
            for template in templates:
                ...
                if matched_points:
                    ...
                    self.click_at(cX, cY)
                    self.click_at(cX, cY)
                    self.logger.log(f'I pressed the button: {cX} {cY}')
...
if __name__ == "__main__":
    ...
    keywords = ["Blum", "Telegram", "P2","Main"]
    ...
    while True:
        try:
            target_percentage = input("Enter a value between 0 and 1 to randomize star clicking, where 1 means collecting all stars. (The choice of size depends on many factors: screen size, window, etc.) I choose values of 0.04 - 0.06 to collect about 140-150 stars. You need to choose the necessary value yourself: ")
    ...
    logger.log('After starting the minigame, press the F6 key on your keyboard')
    target_colors_hex = ["#c9e100", "#bae70e"]
    nearby_colors_hex = ["#abff61", "#87ff27"]
    threshold = 0.9  # Template match threshold
